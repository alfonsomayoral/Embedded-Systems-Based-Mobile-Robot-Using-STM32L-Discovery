...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define tiempo_encendido_zumbador 		500   			// Tiempo para la intermitencia del zumbador = 0,5 segundos

#define tiempo_encendido_sensor     60000   			// Tiempo para empezar el proceso de lectura del sensor de ultrasonidos
															// = 60000 us. Como el TIM2 está configurado a paso por us y el valor
															// máximo de la respuesta del ECHO es 38 ms = 38000 us (si no hay
															// obstáculo), 60000 us vale como intervalo para lanzar de nuevo
															// el proceso de lectura, ya que es mayor que lo que podría ser
															// la respuesta del ECHO + la señal TRIG (10 us) + tiempo del ultrasonido
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
	...
	//CALCULAR DISTANCIA
	 ...
	 unsigned int distancia_tiempo = 0;
	 ...
	 //GIROS
	 unsigned short inicio_mov = 0;
	 unsigned short fin_mov = 0;
	 
	 //PARADA
	 unsigned short inicio_parada= 0;
	 unsigned short fin_parada = 0;

	 //INTERRUPCIONES
	uint8_t fin_trigger=0;
	uint8_t cambio_logico=0;
	uint8_t fin=0;

	//MODULO BLUETOOTH
	uint8_t opciones[7] = "       ";//guardamos los numeros que se envian por bluetooth
	uint8_t mensaje_Transmitir[6] = "      ";//enviar saltos de linea y comienzo de linea
...
// Función de interrupción del TIM3 para crear la intermitencia del zumbador

void TIM3_IRQHandler(void)//TE AÑADIMOS LOS HANDLERS COMO NOS HAS INDICADO PERO POR PROBLEMAS CON EL SENSOR NO HEMOS PODIDO PROBARLAS SOLO EL ZUMBADOR
{
	...
	void TIM2_IRQHandler(void) {
		if ((TIM2->SR & 0x0004)!=0){	//CANAL 2(TOC)-->ENVIAR EL PULSO
				...
				if((TIM2->SR & (1<<1))!=0){//CANAL 1(TIC)-->RECIBIR EL PULSO
						if(cambio_logico==0){
							cambio_logico=1;
						}
				}
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart1, opciones, 1); //implementación de interrupciones a la recepción de la usart1 (modulo bluetooth)
...
//trasmisión sin interrupciones
 ...
 HAL_UART_Transmit(&huart1, mensaje_Transmitir, 1, 10000);
  ...
  //PONGO PA1 (ZUMBADOR) COMO SALIDA DIGITAL
     ...
     GPIOA->BSRR = (1<<1);//pongo un 1 en el zumbador que no suene
  ...
  // Configuración del funcionamiento del contador: PSC, CNT, ARR y CCRx
  	TIM3->PSC = 31999;   // Preescalado = 32000 -> Frecuencia del contador =
  	...
  	TIM3->CCR2 = 500 ;   // Registro donde se guarda el valor que marca la
 ...
 //-------------------------Configuracion TIMER2 CANAl1----------------------------
	...
	unsigned short valor = 0;
     ...
     //------------------------------------------
       ...
       // PB9 TIM4-CH3
          GPIOB->MODER|=0x00000001 << (2*9 +1);  // MODER = 10 (AF) para el bit 7 del
        // puerto B
          GPIOB->MODER&=~(1 << (2*9));
          GPIOB->AFR[1]|=(0x02 << (1*4));        // AFR[0] para decir que el PB7 tiene   ??????????????????
                            // la AF2 (TIM4)
         // PA12 como salida (01) -> Motor izquierdo, IN1 para el control del sentido de giro
       ...
       // 10 pasos
         //TIM4->CCR3 = 0;        // El Duty cycle se pone a 1 inicialmente             CAMBIO
         //TIM4->CCR4 = 0;
         // Selección de IRQ o no: DIER
  ...
  while (1)
  {
	  ...
	  while (opciones[0]==0);       // Si no he recibido nada, espero
	  switch(opciones[0]){

	  	case 0x31://AUTOMATICO-->RECEPCION DE UN 1

	  		HAL_UART_Transmit(&huart1, (unsigned char*) "Modo Auto", sizeof("Modo Auto"), 10000);
	  			  	...
	  			  	//MEDIMOS VALOR DEL POTENIOMETRO Y AJUSTAMOS DC
	  				  ...
	  				  //GIRAR DERECHA
	  				  	  if(girar_derecha){
	  				  		...
	  				  		inicio_parada=TIM3->CNT;
	  				  		fin_parada= inicio_parada + 500;

	  				  		while(TIM3->CNT < fin_parada);



	  				  		//GIRO 90º A LA DERECHA
	  				  		inicio_mov=TIM3->CNT;
	  				  		fin_mov=inicio_mov+500;

	  				  		while(TIM3->CNT < fin_mov){
	  				  			TIM4->CCR4 = 0;
	  				  			GPIOA->BSRR = (1<<11);
	  				  		}
	  				  ...
	  				  //GIRAMOS OTROS 90º A LA IZQUIERDA Y VOLVEMOS POR DONDE VEN�?AMOS
	  				  			if(vuelta_atras){
	  				  				//PARAMOS MEDIO SEGUNDO
	  	...
	  	case 0x32://ADELANTE-->RECEPCIÓN DE UN 2
	  		...
	  		opciones[0]=0; //esperamos a que se envie otro modo
	  	break;
	  	case 0x33://MARCHA ATRÁS-->RECEPCION DE UN 3
...
{
  HAL_UART_Receive_IT(huart, opciones, 1); // Vuelve a activar Rx por haber acabado
 // el buffer
}
